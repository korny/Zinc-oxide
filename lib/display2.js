ZnO.display = {
  ...
  moonsForTileAt: function (x, y) {
    var key = this.keyForTileAt(x, y) + '.moons',
        moons = remember(key) || null;
    
    if (moons === null) {
      this.initRandomForTileAt(x, y);
      moons = remember(key, Random.next(10000) % 6);
    }
    else moons = +moons;
    ...
    if (zoom > 5) {
      ...
      for (x = -2 - offsetX; x < sizeX - offsetX + 1; x++) {
        for (y = -2 - offsetY; y < sizeY - offsetY; y++) {
          ...
          context.drawCircle(drawOffset.x + this.tile.side / 2, drawOffset.y + this.tile.row / 2, this.tile.side / 40 * tile.size, context.fillStyle, 'transparent');
          context.fill();
          if (tile.moons) {
            context.globalAlpha = 1;
            context.fillStyle = 'white';
            context.fillText(tile.moons, drawOffset.x + this.tile.side * 0.7 + this.tile.side / 40 * tile.size, drawOffset.y + this.tile.row / 2);
          }
        }
      }
