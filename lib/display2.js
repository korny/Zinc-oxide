    ...
    if (moons === null) {
      ...
      for (var i = 0; i < number; i++) {
        kind = Random.next(400);
             if ((kind -= 100) < 0) color = 'green';
        else if ((kind -= 100) < 0) color = 'gold';
        else if ((kind -= 100) < 0) color = 'teal';
        else if ((kind -=  60) < 0) color = 'white';
        else if ((kind -=  30) < 0) color = 'cyan';
        else if ((kind -=  10) < 0) color = 'magenta';
        else                        color = null;
        ...
        for (y = -2 - offsetY; y < rows - offsetY; y++) {
          ...
          if (zoom > 25 && tile.moons.length) {
            ...
            for (var moon = tile.moons.length; --moon;) {
              var direction = Math.abs(x - y) % 2 ? 1 : -1;
              var moonOffsetX = drawOffset.x + this.tile.col / 3 + this.tile.side * (0.3 + moon * 0.1) * Math.sin(direction * (moon*moon + (5 - moon) * tile.size/100 * loop.timepoint()));
              var moonOffsetY = drawOffset.y + this.tile.row / 2 + this.tile.side * (0.3 + moon * 0.1) * Math.cos(direction * (moon*moon + (5 - moon) * tile.size/100 * loop.timepoint()));
              context.drawCircle(moonOffsetX, moonOffsetY, this.tile.side * (tile.moons[moon].size / 20) / 40, tile.moons[moon].color, 'transparent');
            }
